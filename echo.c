...
char* nbuff=libc_malloc(sizeof(nbuff)*len*arr_co);
while(arr_co>i){
...
if(libc_sch(args,'[')==1 && libc_sch(args,']')==1){print_item_array(args);}else{id=get_var_id_by_name(args);print_reg_var(id);}	
